...
import { printer } from '@/lib/printer' // Importamos tu librería de impresión

export default function Tickets({ ventas, alTerminar }: any) {
  const hoy = new Date().toLocaleDateString('sv-SE');

  // 1. Filtramos y ORDENAMOS por fecha para que el número sea correlativo
  ...
  // Asignamos el número de orden simple (1, 2, 3...) basado en el orden de llegada
  const listaPedidos = Object.values(pedidosAgrupados).map((pedido: any, index: number) => ({
    ...pedido,
    nroPedido: index + 1
  }));
  ...
  const manejarReimpresion = (e: React.MouseEvent, pedido: any) => {
    ...
    // Al imprimir, añadimos el número de pedido al nombre del cliente
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-4 pb-10">
      <div className="bg-blue-700 p-5 rounded-3xl text-white shadow-lg flex justify-between items-center mx-2">
        <h2 className="font-black uppercase tracking-tighter text-lg italic">Pedidos del Día 📋</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-2">
        {listaPedidos.length > 0 ? (
          // Usamos reverse() solo en la vista para que el cajero vea el más reciente arriba
          [...listaPedidos].reverse().map((pedido: any, i: number) => (
            <div 
              key={i} 
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-[9px] font-black text-blue-400 uppercase leading-none">MESA</p>
                    </div>
              ...
              <div className="p-4 space-y-2">
                {pedido.items.map((item: any, idx: number) => (
                  <div key={idx} className="flex justify-between items-center border-b border-gray-50 pb-1 italic">
                    <span className={`text-xs font-bold uppercase ${pedido.entregado ? 'line-through text-gray-400' : 'text-gray-700'}`}>
                      • {item.nombre}
                    </span>
                    <span className="text-[10px] font-bold text-gray-400">Bs {item.precio.toFixed(2)}</span>
                  </div>
                ))}
              </div>

              <div className="p-4 bg-white flex justify-between items-center border-t border-dashed">
                <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest">Total Consumo</span>
